...
namespace.configure(
    {
        "nautobot_lab": {
            ...
            "python_ver": "3.9",
            "local": False,
            "compose_dir": os.path.join(os.path.dirname(__file__), "nautobot"),
            "compose_files": [
                "docker-compose.yml",
            ],
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_lab.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    compose_command_tokens = [
        "docker-compose",
        f"--project-name {context.nautobot_lab.project_name}",
        f'--project-directory "{context.nautobot_lab.compose_dir}"',
    ]

    for compose_file in context.nautobot_lab.compose_files:
        compose_file_path = os.path.join(context.nautobot_lab.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_lab.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_lab.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_lab"
...
def pylint(context):
    ...
    command = (
        'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_lab'
    )
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_lab/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_lab.local):
